    ...
    >
      ...
      <div className="text-center mt-4">
        <p>
          ...
          <a href="/staff/signup" className="text-blue-500 font-semibold">
            Sign Up
          </a>
        </p>
      </div>
...
const StaffLogin: FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const response = await axios.post("http://localhost:1212/staff/login", {
        userEmail,
        userPassword,
      });
  ...
  const handleGoogleLoginSuccess = async (response: any) => {
    try {
      const res = await axios.post("http://localhost:1212/staff/login", {
