...
export default function AllProductManager() {
  interface Products {
    _id: string;
    Name: string;
    Description: string;
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        let response = await axios.get("http://localhost:1212/manager/products", {
          headers: {
  ...
  const handleDeleteProduct = async (selectedProduct: Products) => {
    ...
    try {
      ...
      await axios.delete(
        `http://localhost:1212/manager/delete-product/${selectedProduct._id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
              ...
              onSubmit={async (e) => {
                ...
                try {
                  ...
                  const response = await axios.patch(
                    `http://localhost:1212/manager/update-product/${selectedProduct._id}`,
                    updatedProduct,
                    {
                      headers: {
                        Authorization: `Bearer ${token}`,
